...
-- Avoids Re-Execution
if _G.SxmSpyExecuted and type(_G.SxmSpyShutdown) == "function" then
	print(pcall(_G.SxmSpyShutdown))
end
...
local Highlight =
	loadstring(
		game:HttpGet("https://raw.githubusercontent.com/Sxm-0/Sxm-Z/main/Spy/Highlight.lua")
	)()

-- Instances:

local SxmSpy2 = Instance.new("ScreenGui")
...
SxmSpy2.Name = "SxmSpy2"
SxmSpy2.ResetOnSpawn = false

Background.Name = "Background"
Background.Parent = SxmSpy2
...
LeftPanel.BackgroundColor3 = Color3.fromRGB(25, 25, 25)--53,52,55
...
_G.SxmSpyCONFIG_MaxRemotes = 500
...
-- whether or not SxmSpy uses 'getcallingscript()' to get the script (default is false because detection)
local useGetCallingScript = false

--- used to enable/disable SxmSpy's keyToString for remotes
...
--- @param args any[]
--- @return string
function SxmSpy:ArgsToString(method, args)
...
--- @param value any
--- @return string
function SxmSpy:ValueToVar(value, variablename)
...
--- @param value any
--- @return string
function SxmSpy:ValueToString(value)
	return v2s(value)
end

--- Generates the SxmSpy function info
--- @param func function
--- @return string
function SxmSpy:GetFunctionInfo(func)
...
function SxmSpy:GetRemoteFiredSignal(remote)
...
--- @param remote Instance
--- @param f function
function SxmSpy:HookRemote(remote, f)
	assert(typeof(remote) == "Instance", "Instance expected, got " .. typeof(remote))
...
--- @param remote any
function SxmSpy:BlockRemote(remote)
	assert(
...
--- @param remote any
function SxmSpy:ExcludeRemote(remote)
	assert(
		typeof(remote) == "Instance" or typeof(remote) == "string",
		"Instance | string expected, got " .. typeof(remote)
	)
...
--- Prevents remote spam from causing lag (clears logs after `_G.SxmSpyCONFIG_MaxRemotes` or 500 remotes)
function clean()
	local max = _G.SxmSpyCONFIG_MaxRemotes
...
--- Executed when the toggle button (the SxmSpy logo) is hovered over
...
--- Called when mouse enters SxmSpy
function mouseEntered()
	local existingCursor = SxmSpy2:FindFirstChild("Cursor")
	while existingCursor do
		existingCursor:Destroy()
		existingCursor = SxmSpy2:FindFirstChild("Cursor")
	end
	...
	customCursor.Image = ""
	customCursor.Parent = SxmSpy2
	UserInputService.OverrideMouseIconBehavior = Enum.OverrideMouseIconBehavior.ForceHide
	RunService:BindToRenderStep("SxmSpy_CURSOR", 1, function()
		if mouseInGui and _G.SxmSpyExecuted then
			...
			RunService:UnbindFromRenderStep("SxmSpy_CURSOR")
		end
	end)
end
...
function backgroundUserInput(input)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 and inResizeRange then
		...
		RunService:BindToRenderStep("SxmSpy_RESIZE", 1, function()
		...
		table.insert(
			...
			UserInputService.InputEnded:Connect(function(inputE)
				if input == inputE then
					RunService:UnbindFromRenderStep("SxmSpy_RESIZE")
				end
			end)
		)
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	remoteFrame.ColorBar.BackgroundColor3 = type == "event" and Color3.new(255, 242, 0) or Color3.fromRGB(99, 86, 45)
	...
	schedule(function()
		...
		if blocked then
			logs[#logs].GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY SxmSpy\n\n"
				.. logs[#logs].GenScript
		end
	end)
...
function t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
	...
	for k, v in pairs(t) do -- iterates over table
		...
		if size > (_G.SxmSpyMaxTableSize or 1000) then
			s = s
				.. "\n"
				.. string.rep(" ", l)
				.. "-- MAXIMUM TABLE SIZE REACHED, CHANGE '_G.SxmSpyMaxTableSize' TO ADJUST MAXIMUM SIZE "
			break
		end
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[Potentially hidden data (tostring in SxmSpy:HookRemote/GetRemoteFiredSignal at your own risk)]]',
				safetostring(v)
			)
		elseif typeof(v) == "userdata" then
...
function formatstr(s, indentation)
	...
	return '"'
		...
		.. (
			reachedMax
				and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE '_G.SxmSpyMaxStringSize' TO ADJUST MAXIMUM SIZE ]]"
			or ""
		)
end
	...
	if not toggle then
		if hookmetamethod then
			...
			if not original then
				warn("SxmSpy: namecall method not found!")
				onToggleButtonClick()
				return
			end
...
function shutdown()
	...
	_G.SxmSpyExecuted = false
end

-- main
if not _G.SxmSpyExecuted then
	local succeeded, err = pcall(function()
		if not RunService:IsClient() then
			error("SxmSpy cannot run on the server!")
		end
		if
			not hookfunction
		...
		then
			...
			shutdown()
			error(
				"This environment does not support method hooks!\n(Your exploit is not capable of running SxmSpy)\nMissing: "
					.. table.concat(missing, ", ")
			)
		end
		_G.SxmSpyShutdown = shutdown
		...
		if syn and syn.protect_gui then
			pcall(syn.protect_gui, SxmSpy2)
		end
		bringBackOnResize()
		SxmSpy2.Parent = --[[gethui and gethui() or]]
			CoreGui
		_G.SxmSpyExecuted = true
	...
	if not succeeded then
		warn(
			"A fatal error has occured, SxmSpy was unable to launch properly.\nPlease DM this error message to @exx#9394:\n\n"
				.. tostring(err)
		)
		SxmSpy2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
...
newButton("Get Script", function()
	...
	if selected then
		setclipboard(SxmSpy:ValueToString(selected.Source))
		TextLabel.Text = "Done!"
	end
end)
...
newButton("Function Info", function()
	...
	if selected then
		if selected.Function then
			codebox:setRaw(
				"-- Calling function info\n-- Generated by the SxmSpy serializer\n\n" .. tostring(selected.Function)
			)
		end
		TextLabel.Text = "Done! Function info generated by the SxmSpy Serializer."
	end
end)
...
newButton("Exclude (i)", function()
	return "Click to exclude this Remote.\nExcluding a remote makes SxmSpy ignore it, but it will continue to be usable."
end, function()
	if selected then
...
newButton("Block (i)", function()
	return "Click to stop this remote from firing.\nBlocking a remote won't remove it from SxmSpy logs, but it will not continue to fire the server."
end, function()
	if selected then
...
newButton("CallingScript", function()
	return string.format(
		"[%s] [UNSAFE] Uses 'getcallingscript' to get calling script for Decompile and GetScript. Much more reliable, but opens up SxmSpy to detection and/or instability.",
...
newButton("GetReturnValue", function()
	...
	if selected then
		codebox:setRaw(SxmSpy:ValueToVar(selected.ReturnValue, "returnValue"))
	end
end)
